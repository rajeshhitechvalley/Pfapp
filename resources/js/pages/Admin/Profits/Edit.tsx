...
const route = (name: string, params?: any) => {
    ...
    if (name === 'profits.update') return `${baseUrl}/admin/profits/${params}`;
...
export default function EditProfit({ profit, users, investments, sales }: EditProfitProps) {
    const { data, setData, put, processing, errors, reset } = useForm({
        user_id: profit.user_id.toString(),
        investment_id: profit.investment_id?.toString() || '',
        sale_id: profit.sale_id?.toString() || '',
        total_profit: profit.total_profit.toString(),
        company_percentage: profit.company_percentage.toString(),
        profit_percentage: profit.profit_percentage.toString(),
        status: profit.status,
        calculation_date: profit.calculation_date,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('profits.update', profit.id), {
            onSuccess: () => reset(),
        });
    };
    ...
    return (
        <AdminLayout title="Edit Profit">
            <Head title={`Edit Profit #${profit.id} - Admin`} />
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                            <Link href={route('profits.show', profit.id)} className="text-gray-600 hover:text-gray-900">
                                <ArrowLeft className="h-5 w-5" />
                            </Link>
                            <div>
                                <h2 className="text-xl font-semibold text-gray-900">Edit Profit Record</h2>
                                <p className="text-sm text-gray-600 mt-1">Update profit distribution record #{profit.id}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
                            ...
                            >
                                ...
                                {processing ? 'Updating...' : 'Update Profit'}
                            </button>
                        </div>
                    </form>
                </div>
